...
```r
info <- subset(sge_failed_codes(), Code %in% unique(codes), select=c(Code, Explanation))
print(info)
# A tibble: 9 × 2
   Code Explanation                                                                                                       
  <int> <chr>                                                                                                             
...
9   100 ran, but killed by a signal (perhaps due to exceeding resources), task died, shepherd died (e.g. node crash), etc.
```
